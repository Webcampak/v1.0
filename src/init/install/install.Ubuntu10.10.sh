...
echo "$(date +'%d %B %Y - %k:%M') xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
...
echo "$(date +'%d %B %Y - %k:%M') /!\     Some manual actions will be required                /!\ "
echo "$(date +'%d %B %Y - %k:%M') xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
echo "$(date +'%d %B %Y - %k:%M') "


echo "$(date +'%d %B %Y - %k:%M') Config: Do you want to [I]nstall or [U]pgrade ?"
echo "$(date +'%d %B %Y - %k:%M') Config: Note: [U]pgrade only work for code upgrades?"
read installtype
...
if [ "$action" = "x" ] ; then
	...
	sudo apt-get update
	echo "$(date +'%d %B %Y - %k:%M') #> sudo apt-get install python-imaging python-opencv libpuzzle-bin rrdtool python-rrdtool wget wput xawtv gphoto2 vsftpd vlc imagemagick apache2 php5 openvpn openssl php5-cli libapache2-mod-php5 lftp mencoder ffmpeg x264 db4.7-util openvpn wput heirloom-mailx python-gdata links vsftpd unzip h264enc php5-gd libavcodec-unstripped-52 libavcodec-extra-52 atomicparsley php5-mysql mysql-server jpeginfo libapache2-mod-auth-mysql pwgen"	
	if [ "$installtype" = "I" ] ; then
		echo "$(date +'%d %B %Y - %k:%M') System: You will have to define Mysql root password"
		read
	fi
	sudo apt-get install python-imaging python-opencv libpuzzle-bin rrdtool python-rrdtool motion python-matplotlib python-configobj mpgtx wget wput xawtv gphoto2 vsftpd vlc imagemagick apache2 php5 openvpn openssl php5-cli libapache2-mod-php5 lftp mencoder ffmpeg x264 db4.7-util openvpn wput heirloom-mailx python-gdata links vsftpd ncftp unzip h264enc php5-gd python-setuptools libavcodec-extra-52 atomicparsley php5-mysql mysql-server jpeginfo libapache2-mod-auth-mysql pwgen streamer 
