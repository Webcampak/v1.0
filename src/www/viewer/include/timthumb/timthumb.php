...
include("../login.php");
include("../witofunctions.php");

include("../allowedsources.php");
...
define ('DIRECTORY_CACHE', $config_directory . "themes/default/cache/timthumb_cache/");		// cache directory
define ('DIRECTORY_TEMP', $config_directory . "themes/default/cache/timthumb_tmp/");		// temp directory
define ('MAX_WIDTH', 100000);					// maximum image width
define ('MAX_HEIGHT', 100000);				// maximum image height
...
$sourceid = get_request ('s', '');
$sourceallowed = 0;
for ($i=0;$i<count($resultlistallowedcameras);$i++) {
	if ($sourceid == $resultlistallowedcameras[$i]["ssourceid"]) {
		$sourceallowed = 1;
	}
}
if ($sourceallowed == 0) {	
...
$src = $config_websourcesdirectory . "source" . $sourceid . "/" . $src;
